...
(function(){
  ...
  // ---------- materials ----------
  const mat={
    contour:new THREE.LineBasicMaterial({color:0xffffff,transparent:true,opacity:.40,vertexColors:true,depthTest:false,depthWrite:false}),
  ...
  const figUni={ uTime:{value:0}, uSwell:{value:0}, uMode:{value:0},
  ...
  mat.contour.onBeforeCompile=(sh)=>{
    ...
    sh.uniforms.uBreeze=figUni.uBreeze; sh.uniforms.uWind=figUni.uWind;
    sh.vertexShader='uniform float uTime;uniform float uSwell;uniform float uMode;uniform vec2 uFig;uniform float uInner;uniform float uOuter;uniform float uBreeze;uniform vec2 uWind;\n'+sh.vertexShader;
  ...
  // real contours, each FLAT at its surveyed elevation; faded out beyond the parcel
  SC.contours.forEach(c=>{ const y=c.z*MULT, a=[], col=[];
    const addv=(px,py)=>{ a.push(px-750,y,py-525); const cc=fadeCol(px,py); col.push(cc[0],cc[1],cc[2]); };
  ...
  (function(){ const raw=G.lake[0].match(/-?[\d.]+/g).map(Number); const C=[];
    for(let i=0;i+1<raw.length;i+=2)C.push(new THREE.Vector2(raw[i],raw[i+1])); if(C.length<3)return;
  ...
  roadLabel('GOWARD RD', 669, 302, 360, {rot:5.9, h:26});       // aligned to north frontage (seg5), tucked just outside
  roadLabel('ECHO DR', 1014, 564, 120, {rot:-40.4, h:22});      // seated on the long SE property line (139.6° axis), running SW→NE
  ...
  // ---------- loop / resize ----------
  window.__v={scene,camera,controls,renderer,P3,ground,elevAt,MULT}; window.__liftoff=kinhinLiftoff;
  ...
  addEventListener('resize',()=>{ camera.aspect=innerWidth/innerHeight; camera.updateProjectionMatrix(); renderer.setSize(innerWidth,innerHeight); composer.setSize(innerWidth,innerHeight); var ov=(mode==='overview'&&!introActive&&!flying); fitOverview(1.0); if(ov)applyPose(OVR); });
