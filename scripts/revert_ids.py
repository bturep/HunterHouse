#!/usr/bin/env python3
"""
REVERT script for the ID rename migration.

Reads the same mapping file as rename_ids.py and does the inverse:
  - Restores P2 from old_id
  - Restores P96 from old_preview
  - Restores P95 from old_master
  - Removes P97 entries that were added by rename_ids.py

Only run this if something went wrong with rename_ids.py.

Usage:
  python3 scripts/revert_ids.py --dry-run
  python3 scripts/revert_ids.py
"""
...
def remove_p97_claim(s, token, qid, old_id, dry_run):
    """Remove the P97 claim that matches old_id (added by rename_ids.py)."""
    ...
    for claim in claims:
        ...
        if val == old_id:
            ...
            s.post(API, data={"action":"wbremoveclaims",
                               "claim":claim["id"],"token":token,"format":"json"})
            return
...
def main():
    ...
    print(f"{'DRY RUN — ' if args.dry_run else ''}REVERT: {len(rows)} items\n")

    if not args.dry_run:
        confirm = input("This will REVERT the ID rename. Type YES to confirm: ")
