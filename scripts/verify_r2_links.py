#!/usr/bin/env python3
"""
Image / PDF pipeline integrity check — addresses ARCHITECTURE.md §11.3.

Background: ingest scripts upload to R2 via rclone, then write the
resulting URL into Wikibase (P95 master, P96 preview, P143 access copy).
A silent rclone failure or a later R2 rename leaves a 404-pointing
claim with no canary anywhere. This script SPARQLs every URL claim out
of Wikibase and HEAD-requests each one against R2.

Read-only. No bot credentials needed (SPARQL + HEAD only). Sequential
HEADs are intentionally polite (50–150 ms per request × ~700 URLs ≈
~1 min on this catalogue) — no concurrency complexity unless the
catalogue grows by an order of magnitude.

Usage:
    python3 scripts/verify_r2_links.py
    python3 scripts/verify_r2_links.py --pid P96       # one property only
...
def main():
    ...
    ap.add_argument("--pid", choices=sorted(URL_PROPERTIES.keys()), action="append",
                    help="restrict to one or more PIDs (default: all)")
