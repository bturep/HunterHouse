...
> This is a verbatim freeze of `CLAUDE.md` at **browse.html v1.06.00 / next.html v1.07-test.01** (2026-05-20).
> It covers the project history **v1.05.00 → v1.06.00 promotion** (the era after `CLAUDE_archive_v1.05.md`).
> It is superseded by the live `CLAUDE.md`, which carries all current reference, the
> Active-context marker, the Pending block, and a condensed digest of the sessions below.
> Consult this file for full historical detail only. Never edit it; never load it as the
> working memory. Full history beyond this also exists in git.

---

# CLAUDE.md — Hunter House Foundation session context
# (FROZEN snapshot — see banner above. Earlier still: CLAUDE_archive_v1.05.md, CLAUDE_archive_v1.02.md)
...
- *Status: Phase 1 BUILT* (next.html v1.06-test.06, **chooser relocated to the filter panel in v1.06-test.22**). Seed `curations/index.json` (8 curators) + `curations/brandon-poole.json` (PLACEHOLDER content — real ids/intro/notes/bio/url pending from Brandon; swap-in needs **no rebuild**; the other 7 are name-only entries — clicking them throws the existing "Curation unavailable" toast). **Curator chooser** = a *row in the Browse filter panel, above Collection* (rendered by `renderFilterPanel`); chips are `.fp-chip[data-cur-slug]` styled `pc-indigo`, click → `openCuratorPane(slug)`. Row hidden in-mode. (Earlier `#curated-bar` slim bar above filters was removed at Brandon's request.) Threshold overlay `#curator-pane` (about-pane pattern, LENS_BLURB + bio + link + "Continue to exhibition"); `state.curation` drives an `applyFilters` early-return (authored membership, `pos` order, sort+filter locked with a toast); `#curation-card` intro card above the list with an **exit** button; record pane swaps `renderRN`→`renderCuratorNote`; mark-bar hidden in-mode. All gated behind `state.curation`/file presence (zero change when absent). *Deliberate v1 deviation:* Esc cancels the **threshold overlay** but does **not** exit an active curation (avoids clobbering Esc's zen/search/overlay duties) — leaving the lens is the always-visible card **exit** button. Revisit if Brandon wants Esc-to-exit.
...
**⚠ Prefetch-sync at promotion:** if `next.html`'s `CATALOGUE_QUERY` changed since the last promotion, also sync `index.html`'s prefetch copy of that query. As of v1.06-test.05 it gained `?accessCopy` + `OPTIONAL{?item wdt:P143 ?accessCopy}` (PDF reader) **and** `?rotation` + `OPTIONAL{?item wdt:P144 ?rotation}` (display rotation). As of v1.06-test.20 it also **loosened P96 from required to OPTIONAL** so stubs without a preview appear in the list. `cp next.html browse.html` carries them into `browse.html`, but `index.html` holds a *separate* copy — unsynced, the PDF "Read" affordance and stored rotation are dark for visitors who enter via the splash prefetch, and `index.html`'s catalogue won't include stubs without P96 either until synced. (A code comment in next.html flags this too.)
...
### 2026-05-19 — Loosen P96 so unscanned stubs appear in browse (next.html v1.06-test.20)
...
### 2026-05-19 — Curated chooser moved into the Browse filter panel (next.html v1.06-test.22)
